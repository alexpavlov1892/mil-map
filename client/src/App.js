...
function App() {
  ...
  const handleSearch = (query, filters) => {
    ...
    if (filters.length > 0) {
      filtered = filtered.filter(loc => 
        loc.target_audience && filters.includes(loc.target_audience)
      );
    }
                ...
                <div className="popUpContainer">
                  ...
                  <div className="campaignInfoContainer">
                    <div className="infoRow">
                      ...
                      <span className="infoValue">{loc.organiser || "N/A"}</span>
                    </div>
                    <hr className="popup-divider" />

                    <div className="infoRow">
                      <span className="infoLabel">Funding:</span>
                      <span className="infoValue">{loc.funding || "N/A"}</span>
                    </div>
                    <hr className="popup-divider" />

                    <div className="infoRow">
                      <span className="infoLabel">Start Date:</span>
                      <span className="infoValue">{loc.start_date || "N/A"}</span>
                    </div>
                    <div className="infoRow">
                      <span className="infoLabel">End Date:</span>
                      <span className="infoValue">{loc.end_date || "N/A"}</span>
                    </div>
                    <hr className="popup-divider" />

                    <div className="infoRow">
                      <span className="infoLabel">Target Audience:</span>
                      <span className="infoValue">{loc.target_audience || "N/A"}</span>
                    </div>
                    <hr className="popup-divider" />
                  </div>
                  ...
                  <button className="closeButton" onClick={() => setActivePopup(null)}>Close</button>
                </div>
              </Popup>
            )}
          </Marker>
